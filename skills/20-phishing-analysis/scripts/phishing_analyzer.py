#!/usr/bin/env python3
"""
Phishing Analyzer - Extract IOCs and analyze an .eml file.
Standalone, OpenClaw-adapted from https://github.com/njodzela/phishing-analyzer
"""
...
def check_abuseipdb(ip: str, api_key: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {"status": "success", "abuse_score": data.get('abuseConfidenceScore', 0), "country": data.get('countryCode', 'Unknown'), "isp": data.get('isp', 'Unknown')}
        else:
            return {"status": "error", "reason": f"HTTP {response.status_code}"}
    except Exception:
...
def calculate_risk(parsed_data: Dict[str, Any], vt_results: Dict[str, Any], ip_results: Dict[str, Any]) -> Any:
    score = 0
    recs = []
...
"""
    ...
    html += """
        ...
        <footer style="margin-top: 30px; font-size: 0.9em; color: #777;">Generated by Phishing Analyzer</footer>
    </div>
</body>
</html>
"""
